  ...
  return (
    ...
    >
      ...
      <div className="w-full h-full max-w-6xl mx-auto overflow-y-auto no-scrollbar flex flex-col md:flex-row gap-8 md:gap-16 pt-12 md:pt-0">
        {/* Media Side */}
        <div className="w-full md:w-1/2 flex flex-col gap-6 items-center justify-start overflow-y-auto no-scrollbar pb-12 pt-4">
          {project.content && project.content.length > 0 ? (
            project.content.map((url, idx) => {
              if (url.toLowerCase().endsWith('.pdf')) {
                return (
                  <motion.iframe
                    ...
                    key={idx}
                    src={`${url}#view=FitH`}
                    className="w-full h-[600px] md:h-[800px] rounded-sm shadow-sm border border-gray-200"
